...
| `sandbox` | 2,000 | Cross-platform sandbox abstraction. Compiles to the macOS DYLD interpose, Linux eBPF, or Windows Detours backend depending on target. Produces a pathset (read set + write set) per task. |
...
| `sandbox-windows-detours` | — (planned) | Windows `cdylib`. Detours-based inline patching of `CreateFileW` / `NtCreateFile` / `ReadFile` / `WriteFile` / `FindFirstFileW` and the rest of the file-access API set. Injected into a suspended child via `DetourCreateProcessWithDllsW`; emits binary `FileAccessEvent` records over a named pipe (`\\.\pipe\rage_sandbox_{pid}_{uid}`) to the parent. Uses `detour-rs` for inline patching to keep the C++ surface out of the build; per-crate `#![allow(unsafe_code)]` (the workspace otherwise forbids unsafe). **Planned** — design described in [`SANDBOX.md`](SANDBOX.md). |
...
### 4. Sandbox — `sandbox`, `sandbox-macos-dylib`, `sandbox-linux-ebpf`, `sandbox-windows-detours` (planned)

A task runs in a child process with the sandbox attached. On macOS, the child inherits `DYLD_INSERT_LIBRARIES=/path/to/librage-sandbox.dylib` and a Mach-O `__DATA,__interpose` table redirects libc file-access syscalls to logging shims. On Linux, an eBPF program is attached to `sys_enter_openat`, `sys_enter_read`, `sys_enter_stat`, etc., filtered by PID. On Windows (planned), the parent launches the child suspended via `DetourCreateProcessWithDllsW`, which injects a `cdylib` that uses Microsoft Detours to inline-patch `CreateFileW`, `NtCreateFile`, `ReadFile`, `WriteFile`, `FindFirstFileW`, and the rest of the file-access API set; events flow back over a named pipe. All three backends produce the same output: a `Pathset { reads: BTreeSet<PathBuf>, writes: BTreeSet<PathBuf> }`.
...
- **Sandbox-as-correctness-primitive** — BuildXL (Detours / VFS); rage's implementation differs by OS (DYLD on macOS, eBPF on Linux, Detours on Windows — same mechanism BuildXL uses, narrower scope).
